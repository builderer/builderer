...
class MsBuildProject:
    ...
    def __init__(self, config: Config, workspace: Workspace, package: Package, target: BuildTarget):
    ...
    def generate_filters(self):
        ...
        xproj.setAttribute("xmlns", "http://schemas.microsoft.com/developer/msbuild/2003")
        ...
        for file in all_files:
            ...
            xfile.setAttribute("Include", as_msft_path(os.path.relpath(file, self.project_root)))
            append_text_element(xfile, "Filter", as_msft_path(os.path.relpath(file.parent, common_dir)))
        # write
    ...
    def _append_project_configurations(self, xparent: Node):
        ...
        for config in self.build_configs:
            ...
            xprojconfig.setAttribute("Include", f"{config.build_config}|{config.architecture}")
    ...
    def _append_dependencies(self, xparent: Node):
        ...
        for dep in deps:
            ...
            xref.setAttribute(
                "Include",
                as_msft_path(os.path.relpath(get_vcxproj_path(self.base_config, dep), self.project_root))
            )
    ...
    def _append_source_files(self, xparent: Node, group_name: str, files: List[str]):
        ...
        for file in files:
            append_element(xgroup, msvc_file_rule(Path(file))).setAttribute(
                "Include",
                as_msft_path(os.path.relpath(file, self.project_root))
            )

    def _append_config_properties(self, xparent: Node, config: Config):
        xgroup = append_element(xparent, "PropertyGroup")
        xgroup.setAttribute("Condition", f"'$(Configuration)|$(Platform)'=='{config.build_config}|{config.architecture}'")
        ...
        if isinstance(self.target, CCBinary):
            out_path = as_msft_path(
                os.path.relpath(
                    os.path.dirname(resolve_conditionals(config=config, value=self.target.output_path)),
                    self.project_root
                )
            )
            append_text_element(xgroup, "OutDir", f"$(ProjectDir)\\{out_path}\\")
        else:
            append_text_element(xgroup, "OutDir", "$(ProjectDir)\\.lib\\$(MSBuildProjectName)\\$(Platform)-$(Configuration)\\")
        append_text_element(xgroup, "IntDir", "$(ProjectDir)\\.obj\\$(MSBuildProjectName)\\$(Platform)-$(Configuration)\\")
    ...
    def _append_local_app_data_platform(self, xparent: Node, config: Config):
        ...
        xsheet.setAttribute("Condition", f"'$(Configuration)|$(Platform)'=='{config.build_config}|{config.architecture}'")
        xprop = append_element(xsheet, "Import")
        xprop.setAttribute("Project", "$(UserRootDir)\Microsoft.Cpp.$(Platform).user.props")
        xprop.setAttribute("Condition", "exists('$(UserRootDir)\Microsoft.Cpp.$(Platform).user.props')")
    ...
    def _append_compile_config(self, xparent: Node, config: Config):
        ...
        # Defines...
        defines = [*resolve_conditionals(config=config, value=self.target.private_defines)]
        if isinstance(self.target, CCLibrary):
            defines.extend(resolve_conditionals(config=config, value=self.target.public_defines))
        defines.extend([
            define
        ...
        if isinstance(self.target, (CCLibrary, CCBinary)):
            includes.extend([
                ...
                for include in resolve_conditionals(config=config, value=self.target.private_includes)
            ])
        if isinstance(self.target, CCLibrary):
            includes.extend([
                as_msft_path(os.path.relpath(include, self.project_root))
                for include in resolve_conditionals(config=config, value=self.target.public_includes)
            ])
        includes.extend([
            as_msft_path(os.path.relpath(include, self.project_root))
            for _,dep_target in self.workspace.all_dependencies(self.package, self.target)
            if isinstance(dep_target, CCLibrary)
            for include in resolve_conditionals(config=config, value=dep_target.public_includes)
        ])
        append_text_element(xcompile, "AdditionalIncludeDirectories", ";".join(includes))
    ...
    def _append_project(self, xparent: Node):
        ...
        append_element(xproj, "Import").setAttribute("Project", "$(VCTargetsPath)\Microsoft.Cpp.Default.props")
        ...
        append_element(xproj, "Import").setAttribute("Project", "$(VCTargetsPath)\Microsoft.Cpp.props")
        ...
        # Import cpp targets
        append_element(xproj, "Import").setAttribute("Project", "$(VCTargetsPath)\Microsoft.Cpp.targets")