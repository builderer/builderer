...
def load_user_module(ctx: Union[ConfigContext, RulesContext, BuildContext]):
    ...
    spec = spec_from_loader(module_name, SourceFileLoader(module_name, str(module_path)))
...
def load_recursive_contexts(
    ...
    contexts = [
        ctx_type(root)
        for root in generate_roots(workspace_root, ctx_type.FILENAME)
    ]
    for ctx in contexts:
...
class Workspace:
    ...
    def find_target(self, name: str, outer: Optional[Package]) -> tuple[Package,Target]:
    ...
    def direct_dependencies(self, package: Package, target: Target) -> Iterator[tuple[Package,Target]]:
        for dep in target.deps:
            yield self.find_target(dep, package)

    def all_dependencies(self, package: Package, target: Target) -> Iterator[tuple[Package,Target]]:
        direct = [
            (p,t)
            for p,t in self.direct_dependencies(package, target)
        ]
    ...
    def _configure_sandbox(self, config: Config, target: Target):
        ...
        target.sandbox_root = Path(config.sandbox_root).joinpath(target.workspace_root, target.name, sandbox_hash).as_posix()
