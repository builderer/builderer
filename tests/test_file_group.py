...
class TestResourceDestinations(unittest.TestCase):
    """resource_destinations: strip_prefix -> (src, dst)"""

    def test_default_strip_prefix_keeps_full_package_relative_path(self):
    ...
    def test_explicit_strip_prefix_drops_prefix_and_preserves_subtree(self):
        fg = _file_group(
            "data",
            strip_prefix="assets",
        ...
        self.assertEqual(
            ...
            [
                ...
                # directly under prefix -> flat
                ("pkg/assets/app.icon", "app.icon"),
            ],
        )

    def test_strip_prefix_matching_normalizes_dotdot_and_trailing_slash(self):
    ...
    def test_source_outside_strip_prefix_errors(self):
        fg = _file_group("data", strip_prefix="assets", srcs=["pkg/other/x.bin"])
        with self.assertRaisesRegex(ValueError, "not under"):
            fg.resource_destinations()


class TestAppleApplicationResources(unittest.TestCase):
    """apple_application: merge + collision across file_groups"""

    def test_resolve_resources_merges_groups_sorted_by_destination(self):
        tex = _file_group(
    ...
    def test_resolve_resources_errors_on_destination_collision(self):
    ...
    def test_resolve_resources_rejects_non_file_group_label(self):
        app, pkg, ws = _app([":bin"], [])  # the binary is not a file_group
        with self.assertRaisesRegex(ValueError, "must reference a file_group"):
            app.resolve_resources(ws, pkg)


class TestMakeResourceCopies(unittest.TestCase):
    """make generator: structure-preserving copies"""

    def test_make_preserves_subdirectories_and_flattens_root_files(self):
        tex = _file_group(
        ...
        self.assertIn("$(MKDIR) $@/Contents/Resources/textures/wood", mk)
        self.assertIn(
            "$(CP) $(WORKSPACE_ROOT)/pkg/assets/textures/wood/oak.texture "
            "$@/Contents/Resources/textures/wood/oak.texture",
            mk,
        )
        # file directly under its strip_prefix lands flat at the resources root
        self.assertIn(
            "$(CP) $(WORKSPACE_ROOT)/pkg/res/app.icon $@/Contents/Resources/app.icon",
            mk,
        )
        # each resource is a prerequisite, so editing it triggers a re-copy
        self.assertIn("$(WORKSPACE_ROOT)/pkg/assets/textures/wood/oak.texture", mk)


class TestXcodeResourceCopies(unittest.TestCase):
    """xcode generator: per-subdir Copy Files phase actually wires the file"""

    def test_xcode_wires_resource_into_a_copy_files_phase_for_its_subdirectory(self):
        tex = _file_group(
            "tex", strip_prefix="assets", srcs=["pkg/assets/textures/wood/oak.texture"]
        )
        ...
        self.assertIn(build_file.id, {ref.id for ref in phase.files})
