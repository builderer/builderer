...
class TestSubsystemSeams(unittest.TestCase):
    def test_conditional_flags_resolve_against_active_config_during_generation(self):
        # I-1: the conditional system and the generator must compose -- a
        # Condition-gated flag reaches the emitted project only for the matching config
        app = make_cc_binary(
            "app",
            srcs=["pkg/m.cpp"],
            cxx_flags=[
        ...
        self.assertTrue(all("-O0" not in a for a in _texts(doc, "AdditionalOptions")))

    def test_multi_arch_multi_config_fans_out_coherently(self):
        # I-2: a config with N architectures x M build_configs produces the full
        # cartesian product, and the .vcxproj and .sln agree on it
        ...
        self.assertEqual(project_configs, expected)
        projects = {
            target_full_name(pkg, app): MsBuildProject(cfg, ws, pkg, app, VS2022)
        }
        buf = io.StringIO()
        MsBuildSolution(cfg, ws, projects, VS2022)._write_solution(buf)
        sln = buf.getvalue()
        for combo in expected:
            # solution-configuration matrix matches
            self.assertIn(f"{combo} = {combo}", sln)

    def test_solution_and_projects_agree_on_dependency_guids_and_paths(self):
        ...
        self.assertIn(f"{lib_guid} = {lib_guid}", sln)
        ...
        self.assertTrue(any("mylib.vcxproj" in r for r in refs))

    def test_makefile_links_dependencies_in_dependency_order(self):
        ...
        self.assertLess(link_line.index("libhigh.a"), link_line.index("liblow.a"))


class TestCombinatorialSweeps(unittest.TestCase):
    """The examples build only samples a few (platform, toolchain) points and proves
    they compile. These sweep every supported configuration and prove the generators
    still produce coherent, internally-consistent output -- catching a break in, say,
    the macos/arm64 or emscripten/wasm32 path that the examples build would miss.
    """

    def test_make_generator_is_coherent_across_supported_configs(self):
        for platform, toolchain, arch in [
            ("linux", "gcc", "x86-64"),
            ("linux", "gcc", "i386"),
            ("macos", "clang", "arm64"),
            ("macos", "clang", "x86_64"),
            ...
            with self.subTest(platform=platform, toolchain=toolchain, arch=arch):
                ...
                mk = buf.getvalue()
                self.assertIn("$(CCLD)", mk)  # the binary link rule is emitted
                self.assertIn("libmylib.a", mk)  # ...and links the dependency's archive

    def test_msbuild_generator_is_coherent_across_architectures(self):
        for arch in ["x64", "arm64"]:
            with self.subTest(arch=arch):
                ...
                self.assertEqual(configs, {f"debug|{arch}"})
                refs = [
                    e.getAttribute("Include")
    ...
    def test_xcode_generator_is_coherent_across_architectures(self):
        for arch in ["arm64", "x86_64"]:
            with self.subTest(arch=arch):
                ...
                self.assertEqual(
                    {t.name for t in proj.nativeTargets}, {"pkg:mylib", "pkg:app"}
                )
